...
cat("You generated",counter,"numbers that sum up to", sum)
```
...
my_bmi<-bmi_calculator(62,105)
cat("Your BMI is",my_bmi[2],"with a value of",round(as.numeric(my_bmi[1]),1))
```
...
```{r}
sum_even <- c(sapply(list, sum)%%2==0)
print(sum_even)
```
...
```{r}
fruit_info[which.max(fruit_info$weight),"fruit"]
```
...
for (i in 1:nrow(fruit_info)){
  if (fruit_info[i,"weight"]>100){
    category<- "heavy"
    cat("The",fruit_info[i,"fruit"],"is",category, "\n")
  }else{
    category<- "light"
    cat("The",fruit_info[i,"fruit"],"is",category,"\n")
  }
